...
void MAPSOPENCVTEST::Core() //main function ,executing when the new datas arrvive.
{
	...
	if(true == DataAvailableInFIFO(Input("Laser_Short"))){
		...
		if(Laser_Short_In_Elt!=NULL){	
			...
			for (int i=0;i<240;i++)
	         {
				 laser_short_raw[i]=Laser_Short_In_Elt->Integer(i+60);//Laser_Short_In_Elt的定义是什么
	...
	if(true == DataAvailableInFIFO(Input("Laser_Long"))){
		...
		if(Laser_Long_In_Elt!=NULL){	
			...
			for (int i=0;i<240;i++)
	         {
				 laser_long_raw[i]=Laser_Long_In_Elt->Integer(i+60);//Laser_Long_In_Elt的定义是什么
	         }
		}
	}
	...
	if(true == DataAvailableInFIFO(Input("Laser_Front"))){
		...
		if(Laser_Front_In_Elt!=NULL){	
			...
			for (int i=0;i<240;i++)
	         {
				 laser_front_raw[i]=Laser_Front_In_Elt->Integer(i+60);
	         }
		}
	}
	...
	for (int i=0;i<240;i++)
	{
     laser_short_x[i]=laser_short_raw[i]*sin((120-i)*3.1415/360)+x_movement;//映射到X，x_movement是安装位置和车的中心线的偏差
	 laser_short_y[i]=laser_short_raw[i]*cos((120-i)*3.1415/360)*laser_short_pitch_cos;//映射到Y，感觉这个公式有问题，有个角度应该是变化的
	 laser_long_x[i]=laser_long_raw[i]*sin((120-i)*3.1415/360)-x_movement;//映射到X
	 laser_long_y[i]=laser_long_raw[i]*cos((120-i)*3.1415/360)*laser_long_pitch_cos;//映射到Y
	 laser_front_x[i]=laser_front_raw[i]*sin((120-i)*3.1415/360);//映射到X
	 laser_front_y[i]=laser_front_raw[i]*cos((120-i)*3.1415/360);//映射到Y
...
laser_long_edge_left=-1;
	...
	else
	{
	 ...
	 if (j==48)
	 {
       laser_long_edge_left=laser_long_x_mean[47];
	   long_left_y=laser_long_y_mean[47];
	 }
	}
...
else
{
	...
	if(MAPS::CurrentTime()>m_nextTimestamp)
	{
		...
		edge_info->Integer(0) = laser_short_edge_right_filter;//0;//laser_short_edge_right_filter;
		edge_info->Integer(1) = laser_short_edge_left_filter;//0;//laser_short_edge_left_filter;
		edge_info->Integer(2) = laser_long_edge_right_filter;
		edge_info->Integer(3) = laser_long_edge_left_filter;
